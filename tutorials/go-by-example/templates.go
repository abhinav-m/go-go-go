...
/*
Go offers built-in support for creating dynamic content or showing customized output to the user
with the text/template package. A sibling package named html/template provides
 the same API but has additional security features and should be used for generating HTML.
*/
func main() {
	t1 := template.New("t1")

	// We can create a new template and parse its value from a string
	...
	// Alternatively use this syntax to panic in case parse returns an error
	...
	*/
	...
	// Helper function to create and return a template
	Create := func(name, t string) *template.Template {
		return template.Must(template.New(name).Parse(t))
	}

	// If the data is a struct, we can use the {{.FieldName}}
	// action to access its fields. The fields should be exported
	// to be accessible when a template is executing
	...
	// Same applies to map, no restriction on the case of key names
	t2.Execute(os.Stdout, map[string]string{
		"Name": "Mickey Mouse",
	})

	// Condition execution for templates
	// - trims whitespace
	// If checks whether passed value is NOT a default value
